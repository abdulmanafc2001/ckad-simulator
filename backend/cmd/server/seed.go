...
func seedQuestions() []*models.Question {
	return []*models.Question{
		...
		},
		{
			...
			Description: "The manifest /root/broken-deploy.yaml is meant to create a Deployment named broken-app in namespace q08, but it is rejected: it declares the wrong apiVersion and is missing the selector that a Deployment requires.",
			Prepare: []models.SetupStep{
				{Name: "create namespace", CommandArgs: "create namespace q08"},
				{Name: "place broken manifest", File: "/root/broken-deploy.yaml", FileContent: "apiVersion: v1\nkind: Deployment\nmetadata:\n  name: broken-app\n  namespace: q08\nspec:\n  replicas: 2\n  template:\n    metadata:\n      labels:\n        app: myapp\n    spec:\n      containers:\n        - name: web\n          image: nginx\n"},
			},
			Task:     "In namespace q08:\n1. Fix /root/broken-deploy.yaml to use apiVersion: apps/v1.\n2. Add a selector whose matchLabels match the pod template labels (app: myapp).\n3. Apply the fixed manifest and make sure Deployment broken-app becomes available with 2 replicas.",
			Hints:    []string{"kubectl apply -f /root/broken-deploy.yaml", "vi /root/broken-deploy.yaml", "kubectl get deploy broken-app -n q08"},
			Solution: "kubectl apply -f /root/broken-deploy.yaml\n# Error: no matches for kind \"Deployment\" in version \"v1\"\n\n# Fix the file (vi /root/broken-deploy.yaml), then re-apply:\napiVersion: apps/v1\nkind: Deployment\nmetadata:\n  name: broken-app\n  namespace: q08\nspec:\n  replicas: 2\n  selector:\n    matchLabels:\n      app: myapp\n  template:\n    metadata:\n      labels:\n        app: myapp\n    spec:\n      containers:\n        - name: web\n          image: nginx",
			Weight:   4,
			Checks: []models.Check{
				{ID: "deploy-exists", Description: "Deployment 'broken-app' exists in namespace q08", Weight: 1, CommandArgs: "get deploy broken-app -n q08 -o name", ExpectSubstring: "broken-app"},
				{ID: "deploy-selector", Description: "Deployment selector matches the template labels", Weight: 1, CommandArgs: "get deploy broken-app -n q08 -o jsonpath={.spec.selector.matchLabels.app}", ExpectRegex: "^myapp$"},
				{ID: "deploy-replicas", Description: "Deployment is configured for 2 replicas", Weight: 1, CommandArgs: "get deploy broken-app -n q08 -o jsonpath={.spec.replicas}", ExpectRegex: "^2$"},
				{ID: "deploy-ready", Description: "All 2 replicas are ready", Weight: 1, CommandArgs: "get deploy broken-app -n q08 -o jsonpath={.status.readyReplicas}", ExpectRegex: "^2$"},
			},
		...
		},
		{
			...
			Description: "Namespace q15 runs a Deployment api-deploy behind Service api-svc on port 8080. The manifest /root/fix-ingress.yaml declares an Ingress, but it is rejected by the API server because its pathType is not one of the allowed values.",
			Prepare: []models.SetupStep{
				{Name: "create namespace", CommandArgs: "create namespace q15"},
				{Name: "setup resources", Namespace: "q15", YAML: "apiVersion: apps/v1\nkind: Deployment\nmetadata:\n  name: api-deploy\n  namespace: q15\nspec:\n  replicas: 1\n  selector:\n    matchLabels:\n      app: api-backend\n  template:\n    metadata:\n      labels:\n        app: api-backend\n    spec:\n      containers:\n        - name: api\n          image: nginx:latest\n          ports:\n            - containerPort: 80\n---\napiVersion: v1\nkind: Service\nmetadata:\n  name: api-svc\n  namespace: q15\nspec:\n  selector:\n    app: api-backend\n  ports:\n    - port: 8080\n      targetPort: 80\n      protocol: TCP\n"},
				{Name: "place broken manifest", File: "/root/fix-ingress.yaml", FileContent: "apiVersion: networking.k8s.io/v1\nkind: Ingress\nmetadata:\n  name: api-ingress\n  namespace: q15\nspec:\n  rules:\n    - http:\n        paths:\n          - path: /api\n            pathType: InvalidType\n            backend:\n              service:\n                name: api-svc\n                port:\n                  number: 8080\n"},
			},
			Task:     "In namespace q15:\n1. Apply /root/fix-ingress.yaml and note the error.\n2. Fix pathType to a valid value (Prefix, Exact or ImplementationSpecific).\n3. Keep the rule routing path /api to Service api-svc on port 8080.\n4. Apply the fixed manifest successfully.",
			Hints:    []string{"kubectl apply -f /root/fix-ingress.yaml", "vi /root/fix-ingress.yaml", "kubectl get ingress api-ingress -n q15 -o jsonpath={.spec.rules[0].http.paths[0].pathType}"},
			Solution: "kubectl apply -f /root/fix-ingress.yaml\n# Error: spec.rules[0].http.paths[0].pathType: Unsupported value: \"InvalidType\"\n\n# Edit the file (vi /root/fix-ingress.yaml) and set pathType: Prefix, then re-apply:\nkubectl apply -f /root/fix-ingress.yaml\n\napiVersion: networking.k8s.io/v1\nkind: Ingress\nmetadata:\n  name: api-ingress\n  namespace: q15\nspec:\n  rules:\n    - http:\n        paths:\n          - path: /api\n            pathType: Prefix\n            backend:\n              service:\n                name: api-svc\n                port:\n                  number: 8080",
			Weight:   4,
			Checks: []models.Check{
				{ID: "ing-exists", Description: "Ingress 'api-ingress' exists in namespace q15", Weight: 1, CommandArgs: "get ingress api-ingress -n q15 -o name", ExpectSubstring: "api-ingress"},
				{ID: "ing-pathtype", Description: "Rule uses a valid pathType", Weight: 1, CommandArgs: "get ingress api-ingress -n q15 -o jsonpath={.spec.rules[0].http.paths[0].pathType}", ExpectRegex: "^(Prefix|Exact|ImplementationSpecific)$"},
				{ID: "ing-path", Description: "Rule routes path /api", Weight: 1, CommandArgs: "get ingress api-ingress -n q15 -o jsonpath={.spec.rules[0].http.paths[0].path}", ExpectRegex: "^/api$"},
				{ID: "ing-backend", Description: "Rule backends onto Service api-svc port 8080", Weight: 1, CommandArgs: "get ingress api-ingress -n q15 -o jsonpath={.spec.rules[0].http.paths[0].backend.service.name}:{.spec.rules[0].http.paths[0].backend.service.port.number}", ExpectRegex: "^api-svc:8080$"},
			},
		...
		},
		{
			...
			Task:        "1. Create a PersistentVolume named task-pv with:\n - Capacity: 1Gi\n - Access mode: ReadWriteOnce\n - hostPath: /mnt/data\n - storageClassName: manual\n\n2. In namespace q21, create a PersistentVolumeClaim named task-pvc with:\n - Request: 500Mi\n - Access mode: ReadWriteOnce\n - storageClassName: manual\n\n3. The PVC should automatically bind to the PV.",
			Hints:       []string{"kubectl apply -f -", "kubectl get pv task-pv", "kubectl get pvc task-pvc -n q21"},
			Solution:    "cat <<'EOF' | kubectl apply -f -\napiVersion: v1\nkind: PersistentVolume\nmetadata:\n  name: task-pv\nspec:\n  capacity:\n    storage: 1Gi\n  accessModes:\n  - ReadWriteOnce\n  hostPath:\n    path: /mnt/data\n  storageClassName: manual\nEOF\ncat <<'EOF' | kubectl apply -f -\napiVersion: v1\nkind: PersistentVolumeClaim\nmetadata:\n  name: task-pvc\n  namespace: q21\nspec:\n  accessModes:\n  - ReadWriteOnce\n  resources:\n    requests:\n      storage: 500Mi\n  storageClassName: manual\nEOF",
			Weight:      4,
			Checks: []models.Check{
		...
		},
		{
			...
			Prepare: []models.SetupStep{
				{Name: "create namespace", CommandArgs: "create namespace q32"},
				{Name: "create backend deployment", Namespace: "q32", YAML: "apiVersion: apps/v1\nkind: Deployment\nmetadata:\n  name: backend\n  namespace: q32\n  labels:\n    app: backend\nspec:\n  replicas: 1\n  selector:\n    matchLabels:\n      app: backend\n  template:\n    metadata:\n      labels:\n        app: backend\n    spec:\n      containers:\n      - name: nginx\n        image: nginx:1.25-alpine\n        ports:\n        - containerPort: 80\n"},
				{Name: "create backend service", Namespace: "q32", YAML: "apiVersion: v1\nkind: Service\nmetadata:\n  name: backend-svc\n  namespace: q32\nspec:\n  selector:\n    app: backend\n  ports:\n  - port: 80\n    targetPort: 80\n"},
			},
			...
			Checks: []models.Check{
				...
				{ID: "check-2", Description: "Pod 'debug-pod' has restartPolicy Never", Weight: 2, CommandArgs: "get pod debug-pod -n q32 -o jsonpath={.spec.restartPolicy}", ExpectRegex: "^Never$"},
			},
		...
		},
		{
			...
			Prepare: []models.SetupStep{
				{Name: "create namespace", CommandArgs: "create namespace q34"},
				{Name: "create deployment scale-app", Namespace: "q34", YAML: "apiVersion: apps/v1\nkind: Deployment\nmetadata:\n  name: scale-app\n  namespace: q34\n  labels:\n    app: scale-app\nspec:\n  replicas: 2\n  selector:\n    matchLabels:\n      app: scale-app\n  template:\n    metadata:\n      labels:\n        app: scale-app\n    spec:\n      containers:\n      - name: nginx\n        image: nginx:1.25-alpine\n"},
			},
			...
			Checks: []models.Check{
				...
				{ID: "check-2", Description: "Deployment 'scale-app' is scaled to 6 replicas", Weight: 2, CommandArgs: "get deployment scale-app -n q34 -o jsonpath={.spec.replicas}", ExpectRegex: "^6$"},
			},
		...
		},
		{
			...
			Checks: []models.Check{
				...
				{ID: "check-2", Description: "Pod 'fixed-pod' fits the quota (CPU request <= 100m)", Weight: 2, CommandArgs: "get pod fixed-pod -n q47 -o jsonpath={.spec.containers[0].resources.requests.cpu}", ExpectRegex: "^([0-9]|[1-9][0-9]|100)m$"},
			},
		...
		},
		{
			...
			Hints:       []string{"kubectl expose deployment api-deploy --name=api-svc --port=80 -n q71", "kubectl get svc api-svc -n q71 -o wide"},
		...
		},
		{
			...
			Prepare: []models.SetupStep{
				{Name: "create namespace", CommandArgs: "create namespace q78"},
				{Name: "create secure-web deployment", Namespace: "q78", YAML: "apiVersion: apps/v1\nkind: Deployment\nmetadata:\n  name: secure-web\n  namespace: q78\n  labels:\n    app: secure-web\nspec:\n  replicas: 1\n  selector:\n    matchLabels:\n      app: secure-web\n  template:\n    metadata:\n      labels:\n        app: secure-web\n    spec:\n      containers:\n      - name: nginx\n        image: nginx:1.25-alpine\n        ports:\n        - containerPort: 80\n"},
				{Name: "create secure-web service", Namespace: "q78", YAML: "apiVersion: v1\nkind: Service\nmetadata:\n  name: secure-web-svc\n  namespace: q78\nspec:\n  selector:\n    app: secure-web\n  ports:\n  - port: 443\n    targetPort: 80\n"},
				{Name: "create tls secret", Namespace: "q78", YAML: "apiVersion: v1\nkind: Secret\nmetadata:\n  name: tls-secret\n  namespace: q78\ntype: kubernetes.io/tls\ndata:\n  tls.crt: \n  tls.key: \n"},
			},
			...
			Checks: []models.Check{
				...
				{ID: "check-2", Description: "Ingress 'secure-ingress' terminates TLS with secret 'tls-secret'", Weight: 2, CommandArgs: "get ingress secure-ingress -n q78 -o jsonpath={.spec.tls[0].secretName}", ExpectRegex: "^tls-secret$"},
			},
